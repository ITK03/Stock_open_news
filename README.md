...
3. **Actions** が `*/10`(平日 JST 10:00-19:00 目安)で自動実行し、データを更新・コミット。
   - private リポジトリは無料枠 **2000分/月**。間隔は `poll.yml` の `cron` で調整。
   - **public** にすると Actions 無制限無料 → `*/5 0-9 * * 1-5` 等に上げて高頻度化可。
