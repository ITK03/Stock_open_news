...
## 雑学 (Trivia / trivia-app が読む)

雑学アプリ用の配信データ。開示レーダーとはパイプラインが独立している。
**100万件規模を前提**にしており、クライアントは全件を取得しない(できない)。

### 入力: `data/trivia_source.jsonl`
1行1件の JSON Lines。100万件をメモリに載せずに流すためこの形式。
```json
{ "id": 1, "setup": "実は…", "punchline": "オナラを我慢すると口から出る" }
```
`setup_text` / `punchline_text` の長いキーでも読める。`punchline` が空の行と
`id` が無い行、壊れた行は落として続行する。`id` 重複は先勝ちで除去。

### 出力: 配信シャード (`src/trivia/build.py` が生成)
`dist/trivia/` 以下。**リポジトリでは追跡しない**(100万件で150MB級。
disclosures.json と同じ履歴肥大化を避ける)。

`manifest.json` … **件数に依存せず常に約100バイト**。ここだけ短TTLで配る。
```json
{ "version": 1, "shard_size": 500, "shard_count": 2000, "total": 1000000,
  "built_at": "2026-07-30T01:28:21+00:00" }
```

`shards/NNNNNN.json` … 1シャード = `shard_size` 件の配列。
キーは配信バイト削減のため1文字(100万件で約25MB差)。
```json
[ { "i": 1, "s": "実は…", "p": "オナラを我慢すると口から出る" } ]
```
`i`=id / `s`=setup(前フリ) / `p`=punchline(オチ)

### 配信の約束
- **シャードは不変**。末尾追記だけなら既存シャードの内容は変わらないので
  `Cache-Control: public, max-age=31536000, immutable` を付けられる。
- 既存シャードの内容が変わった時だけ `version` が上がる。クライアントは
  シャードURLに `?v={version}` を付けるので、版が上がった時のみ再取得が走る。
- 追記しただけでは `version` を上げない(上げると全キャッシュが無駄になる)。
- クライアントは1セッションで **マニフェスト1回 + シャード数回** しか叩かない。
  ランダムなシャードを引くのでリクエストは全シャードに分散し、
  すべて不変なので CDN エッジで返る = オリジンへの負荷はほぼゼロ。
