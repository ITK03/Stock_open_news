...
SUPPRESSORS: list[tuple[str, int]] = [
    # 譲渡制限付株式報酬(RSU)は役員報酬の少額発行で影響軽微。増資と誤検出しやすいので強く減衰。
    ("譲渡制限付株式報酬", 50),
    ("譲渡制限付株式", 50),
    ("リストリクテッド・ストック", 50),
    # 増資・新株予約権の「事後・続報」(初回announcementではない)
    ("払込完了", 38),
    ("払込日の確定", 38),
    ("払込期日", 32),
    ("発行株式数の確定", 40),
...
# (category, direction) で引き、無ければ (category, "*") を使う。
